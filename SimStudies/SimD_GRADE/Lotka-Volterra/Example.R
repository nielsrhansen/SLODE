...
m <- aim::mak(A = A, B = B, rx0 = reg(lower = -Inf))
...
## Mispecified MAK
A_misp <- rbind(A, diag(d), 
  do.call(rbind, lapply(AB, getElement, "A")))
B_misp <- rbind(B, diag(rep(c(0, 2), k)),
  do.call(rbind, lapply(AB, getElement, "B")) / 2)
m_misp <- mak(A_misp, B_misp)
  ...
  for (n_series in unique(sim_parameters$n_series)) {
    for (tt in seq_along(traj)) {
      ...
      results[[i]] <- apply(eps, 3, function(e) {
        ...
        ### Misp. Process ###
        a <- aim(m_misp, op = opt(y))
        rod <- rodeo(a)
        
        netw_guess  <- apply(a$params[[1]] != 0, 2, function(ind) {
          A_guess <- A_misp[ind, , drop = FALSE] 
          B_guess <- B_misp[ind, , drop = FALSE] 
          Matrix(t(abs(B_guess - A_guess)) %*% abs(A_guess)) != 0
        })
        TFP <- lapply(netw_guess, function(n) {
          c(FPR = sum(n & !netw_true), TPR = sum(n & netw_true))
        ...
        # plot(TFP_aim, type = "l", ylim = c(0, 1), xlim = c(0, 1))
        # lines(TFP_grade, col = "red")
        # abline(a = 0, b = 1)
        
        print(rep(i, 100))
        print(ss <<- ss + 1)
        
        return(list(aim = TFP_aim, misp = TFP_misp, grade = TFP_grade))
      })
      save(results, file = "results02.RData")
    }
  }
}
...
pdf(paste0("Lotka-Volterra/grade_rocs.pdf"), paper = "a4")
...
pdf(paste0("Lotka-Volterra/grade_rocs_single.pdf"), height = 8, width = 8) 
print(gg_save)
dev.off()
